...
sys.path.append('./')
from src.C2_Hotspot_test import predict_annotated_wsi
import argparse
import time
...
parser.add_argument('-input-path', metavar='wsi-input-path',
                    ...
                    default='./data/wsi/hotspot_slide',
                    help='designate a directory input path')
parser.add_argument('-input-annotation-path', metavar='-input-annotation-path',
                    type=str, dest='input_annotation_path',
                    default='./data/wsi/hotspot_annotation',
                    help='designate a directory hotspot annotation path')
...
input_annotation_file_path = os.path.join(args.input_annotation_path,
                                          '{}.npy'.format(args.filename))
model_file_path = os.path.join(args.model_path,
                               '{}.pth'.format(args.model_name))
predict_annotated_wsi(input_file_path,
                      input_annotation_file_path,
                      args.output_path,
                      model_file_path,
                      visualize=True)
